...
public class Utils
{
    ...
    public class Sql
    {
        ...
        public static int UpdateSelfSigned(serverType table, string searchTerm, byte[] selfSignedCert, int duration, int serialNumber)
        {
            try
            {
                SqliteConnectionStringBuilder _connectionString = new SqliteConnectionStringBuilder();
    ...
    public class Certs
    {
                    ...
                    using (RSA rsa = RSA.Create())
                    {
                        ...
                        switch (table)
                        {
                            case serverType.ca:
                                ...
                                Certificate = intermediateRequest.CreateSelfSigned(DateTimeOffset.Now, DateTimeOffset.Now.AddMonths(Convert.ToInt32(dictCaDetails["ss_duration"])));
                        ...
                        //Certificate = new X509Certificate2(dictCaDetails["pub_key"], issuerPassword, X509KeyStorageFlags.Exportable);
        ...
        public static X509Certificate2 CreateCertificate(serverType table, string requestPrivKey, X500DistinguishedName distinguishedName, byte[] issuerCert, string issuerPasswd ,int requesterDuration, int requesterSerialNumber)
        {
